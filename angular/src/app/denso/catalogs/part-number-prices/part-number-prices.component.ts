...
import { CatalogServiceProxy, CurrencyDto, PartNumberPriceDto } from '@shared/service-proxies/service-proxies';
...
@Component({
    selector: 'app-part-number-prices',
    templateUrl: './part-number-prices.component.html',
    styleUrls: ['./part-number-prices.component.css'],
    animations: [appModuleAnimation()],
})
export class PartNumberPricesComponent extends AppComponentBase implements OnInit {
    partNumberPrices: PartNumberPriceDto[] = [];
    partNumberPriceChanges: PartNumberPriceDto = new PartNumberPriceDto();
    ...
    public onInitNewRow(event: any): void {
        this.partNumberPriceChanges = new PartNumberPriceDto();
    }

    public onEditingStart(event: any): void {
        this.partNumberPriceChanges = new PartNumberPriceDto();
        let newPartNumberPrice = new PartNumberPriceDto(); //cloneDeep(this.unitMeasureSatChanges);
        newPartNumberPrice = this.partNumberPrices.find((pn: PartNumberPriceDto) => pn.id === event.data.id);
        this.partNumberPriceChanges.id = event.data.id;
        this.partNumberPriceChanges.customerId = newPartNumberPrice.customerId;
        this.partNumberPriceChanges.partNumberId = newPartNumberPrice.partNumberId;
    ...
    public refresh(): void {
        ...
        this._catalogService
            .getPartNumberPrices(undefined)
            .pipe(
                finalize(() => {
                    this.isTableLoading = false;
                })
            )
            .subscribe((response: PartNumberPriceDto[]) => {
    ...
    public onSavingPartNumber(e: any): void {
        ...
        if (change || this.isEditing) {
            ...
            if (change != undefined && change.type === 'remove') {
                ...
                this._catalogService
                    .createOrUpdatePartNumberPrice(this.partNumberPriceChanges)
                    .pipe(
                        finalize(() => {
                            // this.saving = false;
