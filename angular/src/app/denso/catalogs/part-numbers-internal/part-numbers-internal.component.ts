...
import {
    ...
    PartNumberInternalXlsxDto,
    PartNumberInternalResXlsxDto,
...
@Component({
    selector: 'app-part-numbers-internal',
    templateUrl: './part-numbers-internal.component.html',
    styleUrls: ['./part-numbers-internal.component.css'],
    animations: [appModuleAnimation()],
})
export class PartNumbersInternalComponent extends AppComponentBase implements OnInit {
    @Input() partNumbers: PartNumberInternalDto[] = [];
    @Output() onPartNumberInternalChanged = new EventEmitter<PartNumberInternalDto[]>();

    partNumberChanges: PartNumberInternalDto = new PartNumberInternalDto();
    listPartNumberInternalXlsx: PartNumberInternalXlsxDto[] = [];
    partNumberInternalResXlsxDto: PartNumberInternalResXlsxDto[] = [];
    ...
    public onInitNewRow(event: any): void {
        this.partNumberChanges = new PartNumberInternalDto();
    }

    public onEditingStart(event: any): void {
        this.partNumberChanges = new PartNumberInternalDto();
        let newPartNumber = new PartNumberInternalDto(); //cloneDeep(this.unitMeasureSatChanges);
        newPartNumber = this.partNumbers.find((pn: PartNumberInternalDto) => pn.id === event.data.id);
        ...
        this.partNumberChanges.price = newPartNumber.price;
        this.Editing = true;
    }
    ...
    public refresh(): void {
        ...
        this._catalogService
            .getPartNumbersInternal(undefined)
            .pipe(
                finalize(() => {
                    this.isTableLoading = false;
                })
            )
            .subscribe((response: PartNumberInternalDto[]) => {
    ...
    public onSavingPartNumber(e: any): void {
        ...
        if (change || this.Editing) {
            ...
            if (change != undefined && change.type === 'remove') {
                ...
                this._catalogService
                    .createOrUpdatePartNumberInternal(this.partNumberChanges)
                    .pipe(
                        finalize(() => {
                            // this.saving = false;
    ...
    public onSaveDataXlsx(event: any): void {
        ...
        if (headers['header1'].length === 8) {
            ...
            if (
                ...
                if (listParts.length > 0) {
                    for (const item of listParts) {
                        try {
                            ...
                            var itemListResponse = new PartNumberInternalXlsxDto();
                            ...
                            this.listPartNumberInternalXlsx.push(itemListResponse);
                        } catch (exception) {
                            throw exception;
                        }
                    }
                ...
                this._catalogService
                    .createOrUpdatePartNumberInternalXLSX(this.listPartNumberInternalXlsx)
                    .pipe(
                        finalize(() => {
                            //this.isTableLoading = false;
                        })
                    )
                    .subscribe((response: PartNumberInternalResXlsxDto[]) => {
                        this.partNumberInternalResXlsxDto = response;
                        var counter = String(this.partNumberInternalResXlsxDto[0].counter);
                        var counterDel = String(this.partNumberInternalResXlsxDto[0].counterDel);
                        var counterIns = String(this.partNumberInternalResXlsxDto[0].counterIns);
                        var counterUpd = String(this.partNumberInternalResXlsxDto[0].counterUpd);
